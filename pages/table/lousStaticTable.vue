<template>
    <view class="tablet-profit-page">
      ...
      <view class="top-toolbar">
        <view class="toolbar-left">
          ...
          <text class="page-title">欠条统计表</text>
        </view>
        ...
        <view class="toolbar-filter">
          ...
          <view class="date-group" v-if="currentTab === 1">
            ...
            <view class="confirm-button" @click="handleCustomDateConfirm">
      ...
      <view class="main-content">
        <view class="table-container">
          <view class="table-header-info">
            <text class="table-title">欠条明细</text>
            <text class="table-subtitle">共 {{ dutyStaticList.length }} 个欠条</text>
          </view>
            ...
            <view class="table-wrapper">
              ...
              <view class="fixed-column">
                ...
                <view class="fixed-header">
                  <text class="header-text">客户名称</text>
                </view>
                ...
                <scroll-view class="fixed-body" scroll-y :style="{ height: tableHeight + 'px' }">
                  <view 
                    class="fixed-row" 
                    v-for="(item, index) in dutyStaticList" 
                    :key="item.id"
                    @click="handleRowClick(item)"
                  >
                    <view class="product-cell">
                      <text class="product-name">{{ item.customerName}}</text>
                    </view>
                  </view>
                  
                  <!-- 空状态 -->
                  <view class="empty-fixed-row" v-if="dutyStaticList.length === 0">
                    <text>暂无数据</text>
                  </view>
                </scroll-view>
              ...
              <view class="scroll-column">
                ...
                <scroll-view class="scroll-header" scroll-x>
                  <view class="scroll-header-content">
                    <view class="scroll-header-cell">欠条金额</view>
					<view class="scroll-header-cell">已还金额</view>
					<view class="scroll-header-cell">未还金额</view>
                    <view class="scroll-header-cell">状态</view>
                    <view class="scroll-header-cell">创建时间</view>
                    <!-- <view class="scroll-header-cell">更新时间</view> -->
                  </view>
                </scroll-view>
                  ...
                  <view class="scroll-body-content">
                    ...
                    >
                      <view class="scroll-cell price-cell">
                        <text class="price-value">{{ item.lousNum}}</text>
                      </view>
					  <view class="scroll-cell price-cell">
					    <text class="price-value">{{  item.paid }}</text>
					  </view>
					  <view class="scroll-cell price-cell">
					    <text class="price-value">{{  item.pendPayment}}</text>
					  </view>
                      <view class="scroll-cell sales-cell">
                        <text class="sales-value" :class="item.pendPayment == 0 ? 'status-paid' : 'status-unpaid'">{{ item.pendPayment == 0 ? '还清' : '未还清' }}</text>
                      </view>
                      <view class="scroll-cell total-cell">
                          <text class="total-value">{{ formatDate(item.createTime) || '-' }}</text>
                        </view>
                    </view>
                    
                    <!-- 空状态 -->
                    <view class="empty-scroll-row" v-if="dutyStaticList.length === 0">
                ...
                <scroll-view class="scroll-footer" scroll-x>
                  <view class="scroll-footer-content">
                    <view class="scroll-footer-cell">
                      <text class="summary-amount">欠条总数: {{ totalSales }}</text>
                    </view>
                    <view class="scroll-footer-cell">
                      <text class="summary-amount-lous">总欠条金额: {{ totalAmount }}</text>
                    </view>
  ...
  <script>
      import lousApi from '../../api/lous/lousApi.js'
  export default {
    ...
    computed: {
      totalSales() {
        return this.dutyStaticList.length // 显示欠条总数
      },
      totalAmount() {
        return this.dutyStaticList.filter(item => item.status === '正常' || !item.status).length // 显示正常欠条数
      }
    },
    mounted() {
    ...
    methods: {
      loadData() {  
                  lousApi.GetBycompanyId(this.currentCompanyId).then(res => {
                      console.log("lous res", res)
                      this.dutyStaticList=res.data;
                  })
              },
      // 工具方法
      formatDate(date) {
