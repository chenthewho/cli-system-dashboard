<template>
    <view class="tablet-profit-page">
      ...
      <view class="top-toolbar">
        <view class="toolbar-left">
          ...
          <text class="page-title">轮值数据统计表</text>
        </view>
      ...
      <view class="main-content">
        <view class="table-container">
          <view class="table-header-info">
            <text class="table-title">数据明细</text>
            <text class="table-subtitle">共 {{ dutyStaticList.length }} 条记录</text>
          </view>
                ...
                <scroll-view class="fixed-body" scroll-y :style="{ height: tableHeight + 'px' }">
                  ...
                  >
                    <view class="product-cell">
                      <text class="product-name">{{ item.operatorName }}</text>
                      <text class="product-no">时间: {{ item.beginTime.replace("T", " ") }}至{{ item.endTime.replace("T", " ") }}</text>
                    </view>
                  </view>
              ...
              <view class="scroll-column">
                ...
                <scroll-view class="scroll-header" scroll-x>
                  <view class="scroll-header-content">
                    <view class="scroll-header-cell">营业额(元)</view>
                    <view class="scroll-header-cell">实际收入（元）</view>
                    <view class="scroll-header-cell">单量</view>
                    <view class="scroll-header-cell">欠单量</view>
                  </view>
                </scroll-view>
                  ...
                  <view class="scroll-body-content">
                    ...
                    >
                      <view class="scroll-cell price-cell">
                        <text class="price-value">{{ formatNumber(item.turnover) }}</text>
                      </view>
                      <view class="scroll-cell sales-cell">
                        <text class="sales-value">{{ item.actualMoney }}</text>
                      </view>
                      <view class="scroll-cell total-cell">
                          <text class="total-value">{{ formatNumber(item.orderNum) }}</text>
                        </view>
                      <view class="scroll-cell total-cell">
                          <text class="total-value">{{ formatNumber(item.debtNum) }}</text>
                        </view>
                    </view>
  ...
  export default {
    ...
    computed: {
      totalSales() {
        return this.dutyStaticList.reduce((sum, item) => sum + (item.orderNum || 0), 0)
      },
      totalAmount() {
        return this.dutyStaticList.reduce((sum, item) => sum + (item.turnover || 0), 0)
      }
    },
    mounted() {
    ...
    methods: {
      loadData() {  
                  tableApi.getDutyStaticByTimeRang(this.currentCompanyId,this.startDate,this.endDate).then(res => {
                      console.log("res", res)
      ...
      handleRowClick(item) {
        uni.showModal({
          title: '轮值详情',
          content: `名称：${item.name}\n营业额：¥${this.formatNumber(item.turnover || 0)}\n实际收入：¥${this.formatNumber(item.actualMoney || 0)}\n单量：${item.orderNum || 0}\n欠单量：${item.debtNum || 0}`,
          showCancel: false
        })
      },
