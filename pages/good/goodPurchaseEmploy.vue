<template>
  <div>
    <div style="display: flex; width: 100%">
      ...
      <div style="flex: 1; background-color: #ffffff; text-align: right">
        <div
          ...
          "
        >
          <div style="font-size: 15rpx; font-weight: bold">{{ currentShipper.shipperName }} — 采购单</div>
          <div style="font-size: 15rpx; font-weight: bold">
            <button v-if="expenseList.length === 0" class="add-expense-btn" @click="showExpenseManageModal">
              添加费用
            </button>
            <view v-else class="expense-label" @click="showExpenseManageModal">
              <text class="expense-label-text"
                >费用({{ expenseList.length }}项)：¥{{ getTotalExpense().toFixed(2) }}</text
              >
            </view>
          </div>
        </div>
        <div :style="{ marginLeft: '5rpx', marginRight: '5rpx', height: windowHeight - 60 + 'px' }">
          ...
          >
            ...
            <div class="total-content">
              ...
              <div class="total-content-item" style="flex: 2"></div>
              <div class="total-content-item" style="flex: 2"></div>
              <div class="total-content-item" style="flex: 2">￥{{ totalAmount() }}</div>
        ...
        <div
          style="position: fixed; bottom: 0; height: 30rpx; background: #ffffff; width: 100%"
      ...
      <view class="expense-modal-wrapper" @click.stop>
        <view class="expense-modal-container">
          ...
          <view class="expense-modal-header">
            ...
            <button
              class="expense-header-btn confirm"
                  ...
                  >
                    <view class="allocation-btn-content">
                      <uni-icons
                        :type="method.icon"
                        size="24"
                        :color="selectedAllocationMethod === method.value ? '#fff' : '#606266'"
                      ></uni-icons>
    ...
    <view v-if="multiLevelPopup.visible" class="multilevel-popup-mask" @click="closeMultiLevelPopup">
      <view class="multilevel-popup-container" :style="multiLevelPopupStyle" @click.stop>
...
export default {
  ...
  methods: {
    ...
    addNewGood() {
      ...
      category
        .CommoditySave3(JSON.stringify(that.newGoodInfo))
        .then(res => {
          uni.showToast({
            icon: 'none',
            title: '添加成功',
    ...
    saveBatchGood() {
      ...
      if (that.BatchCommodityList.length > 0) {
        category
          .CreatePurchaseOrder(JSON.stringify(param))
          .then(res => {
            if (res.code == 200) {
              uni.showToast({
                title: '采购成功',
    ...
    handleMultiLevelSelected(childCommodity) {
      ...
      // 检查是否已经添加
      if (
        this.BatchCommodityList.some(
          x => x.commodityId === childCommodity.commodityId || x.commodityId === childCommodity.id
        )
      ) {
        uni.showToast({
          title: '该商品已添加',
      ...
      var commodityAdded = {
        id: '',
        name: childCommodity.parentName + childCommodity.commodityName,
        commodityId: childCommodity.commodityId || childCommodity.id,
        initMount: 0,
        initWeight: 0,
        fixWeight: childCommodity.initWeight, //单位重量（目前仅用于定装）
        calcType: childCommodity.saleWay === 2 ? 2 : 1,
        saleWay: childCommodity.saleWay,
        incomingPrice: childCommodity.incomingPrice ? childCommodity.incomingPrice : 0,
        originalIncomingPrice: childCommodity.incomingPrice ? childCommodity.incomingPrice : 0, // 保存原始采购单价
        expenseAllocated: 0, // 初始化分摊费用为0
        inboundAmount: 0,
        mount: 0,
      }
    ...
    handleSpecSelected(data) {
      const { commodity, spec } = data
      console.log('选择拆包规格:', commodity, spec)
      // 检查是否已经添加
      if (this.BatchCommodityList.some(x => x.commodityId === spec.id)) {
        uni.showToast({
          title: '该商品已添加',
          icon: 'none',
          duration: 1500,
      ...
      var commodityAdded = {
        id: '',
        name: commodity.commodityName + '-' + spec.specName,
        commodityId: commodity.id,
        mount: 0,
        initMount: 0,
        initWeight: 0,
        fixWeight: commodity.initWeight, //单位重量（目前仅用于定装）
        calcType: commodity.saleWay === 2 ? 2 : 1,
        saleWay: commodity.saleWay,
        incomingPrice: commodity.incomingPrice ? commodity.incomingPrice : 0,
        originalIncomingPrice: commodity.incomingPrice ? commodity.incomingPrice : 0, // 保存原始采购单价
        expenseAllocated: 0, // 初始化分摊费用为0
        inboundAmount: 0,
        spec: spec,
      }
    ...
    createNativeView(left, top, commodity, specList) {
      ...
      zoneTop =
        zoneTop + zoneHeight > plus.screen.resolutionHeight ? plus.screen.resolutionHeight - zoneHeight : zoneTop
      ...
      labelApi.GetExpenseLabel(this.companyId, 2).then(res => {
        if (res.code === 200) {
          ...
          if (this.expenseTypes.length > 0) {
            const validType = this.expenseTypes.find(
              type => type.value === this.selectedExpenseType || type.id === this.selectedExpenseType
            )
            if (!validType) {
              // 当前选中的费用类型无效，设置为第一个
    ...
    confirmExpense() {
      ...
      // 获取费用类型的label（名称）
      const expenseType = this.expenseTypes.find(
        type => type.id === this.selectedExpenseType || type.value === this.selectedExpenseType
      )
...
.znj-add-good-unit-btn.active,
.znj-add-good-unit-btn:hover {
  background: #07c160;
  color: #fff;
  border-color: #07c160;
}
