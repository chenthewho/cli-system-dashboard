  ...
  <view class="customer-container">
    ...
    >
      <view class="customer-list">
        <view
          v-for="item in memberOptionsShow"
          :key="item.id"
    ...
    <div class="bottom-toolbar">
      <div class="stats-section">
        <div class="stat-group">
          ...
          <span class="stat-text"
            >客户数: <strong>{{ totalCount }}</strong></span
          >
        </div>
        <div class="stat-group">
          <span class="stat-text"
            >总欠款: <strong class="debt-text">{{ allDebt.toFixed(1) }}</strong></span
          >
        </div>
        <div class="stat-group">
          <span class="stat-text"
            >总欠筐: <strong>{{ allOweBasket }}</strong></span
          >
        </div>
      </div>
      ...
      <div class="action-section">
        ...
        <button
          ...
          @click="deleteHandle"
          v-if="isEdting"
          :disabled="!isEdting || checkboxValue.length === 0"
        >
          <span>删除</span>
        </button>
...
export default {
  ...
  methods: {
      ...
      member
        ...
        .then(res => {
          if (res.data) {
            ...
            // 使用后端返回的统计数据
            this.allDebt = res.data.TotalDebts || res.data.totalDebts || 0
            this.allOweBasket = res.data.TotalOweBasket || res.data.totalOweBasket || 0
    ...
    deleteHandle() {
      ...
      member
        .memberDelete(JSON.stringify(this.checkboxValue))
        .then(res => {
          uni.showToast({
            title: '删除成功',
            icon: 'none',
