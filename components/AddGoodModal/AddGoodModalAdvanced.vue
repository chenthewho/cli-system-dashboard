    ...
    <view v-if="activeSelect.type" class="native-select-options-global" :style="dropdownStyle" @click.stop>
      ...
      <template v-if="activeSelect.type === 'package'">
        <view
          ...
          :class="{
            active:
              formData.gradeList[activeSelect.index] &&
              formData.gradeList[activeSelect.index].packageType === option.label,
          }"
      ...
      <template v-if="activeSelect.type === 'unit'">
        <view
          v-for="(option, optIndex) in filteredUnitList"
          :key="optIndex"
          class="native-select-item"
          :class="{
            active:
              formData.gradeList[activeSelect.index] &&
              formData.gradeList[activeSelect.index].countUnit === option.value,
          }"
        ...
        <scroll-view scroll-y="true" class="znj-add-good-modal-body">
          <view class="znj-add-good-form">
            ...
            <view class="znj-add-good-row">
              ...
              <input class="znj-add-good-input-name" type="text" placeholder="请输入货品名称" v-model="formData.name" />
            </view>
            ...
            <view class="znj-add-good-row">
              ...
              <div class="znj-add-good-switch-group">
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.stockType === 1 }"
                  @click="formData.stockType = 1"
                >
                  自营
                </button>
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.stockType === 2 }"
                  @click="formData.stockType = 2"
                >
                  代卖
                </button>
              </div>
            ...
            <view class="znj-add-good-row">
              ...
              <div class="znj-add-good-switch-group">
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.isMultiLevel === 1 }"
                  @click="formData.isMultiLevel = 1"
                >
                  是
                </button>
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.isMultiLevel === 0 }"
                  @click="formData.isMultiLevel = 0"
                >
                  否
                </button>
              </div>
            ...
            <view class="znj-add-good-row" v-if="formData.isMultiLevel === 1">
              ...
              <div class="znj-add-good-switch-group">
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.isShareStock === 1 }"
                  @click="setShareStock(1)"
                >
                  是
                </button>
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.isShareStock === 0 }"
                  @click="setShareStock(0)"
                >
                  否
                </button>
              </div>
            ...
            <div v-if="formData.isMultiLevel === 1" class="znj-add-good-grade-area">
              ...
              <view class="znj-grade-table">
                ...
                <view class="znj-grade-table-row" v-for="(grade, index) in formData.gradeList" :key="index">
                  ...
                  <view class="znj-grade-td znj-grade-td-action">
                    <button
                      class="znj-grade-delete-btn"
                      @click="removeGrade(index)"
                      v-if="formData.gradeList.length > 1"
                    >
              ...
              <view class="znj-add-grade-btn-wrapper" @click="addGrade">
                <text class="znj-add-grade-btn"> + 新增货品等级 </text>
              </view>
            </div>
            ...
            <view class="znj-add-good-row" v-if="formData.isMultiLevel == 0">
              ...
              <view class="znj-add-good-btn-group">
                <button
                  class="znj-add-good-type-btn"
                  :class="{ active: formData.saleWay === 1 }"
                ...
                <button
                  class="znj-add-good-type-btn"
                  :class="{ active: formData.saleWay === 2 }"
                ...
                <button
                  class="znj-add-good-type-btn"
                  :class="{ active: formData.saleWay === 3 }"
                  @click="changeSaleType(3)"
                >
                  按件拆包
                </button>
                <button
                  class="znj-add-good-type-btn"
                  :class="{ active: formData.saleWay === 4 }"
                  @click="changeSaleType(4)"
                >
                  散装
                </button>
              </view>
            ...
            <!-- 规格相关 -->
            <div
              v-if="formData.saleWay != 1 && formData.saleWay != 2 && formData.isMultiLevel == 0"
              :class="{ 'znj-add-good-spec-area': formData.saleWay === 3 }"
            >
              <!-- 规格二 -->
              <view class="znj-add-good-row" v-if="formData.specList.length > 1">
                ...
                <div class="znj-add-good-spec-box">
                  ...
                  <input
                    class="znj-add-good-spec-input"
                    type="text"
                    v-model="formData.specList[1].fixcount"
                    placeholder=""
                  />
                </div>
                <selectLayUnit
              ...
              <view class="znj-add-good-row" v-if="formData.specList.length > 2">
                ...
                <div class="znj-add-good-spec-box">
                  ...
                  <input
                    class="znj-add-good-spec-input"
                    type="text"
                    v-model="formData.specList[2].fixcount"
                    placeholder=""
                  />
                </div>
                <selectLayUnit
            ...
            <view class="znj-add-good-row">
              ...
              <div class="znj-add-good-switch-group">
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.showToSale === 1 }"
                  @click="formData.showToSale = 1"
                >
                  是
                </button>
                <button
                  class="znj-add-good-switch-btn"
                  :class="{ active: formData.showToSale === 0 }"
                  @click="formData.showToSale = 0"
                >
                  否
                </button>
              </div>
...
export default {
  ...
  computed: {
    // 完整的单位列表（包含斤和公斤）
    fullUnitList() {
      return [...this.unitList, { label: '斤', value: '斤' }, { label: '公斤', value: '公斤' }]
    },
    // 下拉框样式
    dropdownStyle() {
      return {
    ...
    // 根据销售方式过滤单位列表
    filteredUnitList() {
      if (this.formData.saleWay === 4) {
        // 散装模式下只显示斤和公斤
        return this.fullUnitList.filter(unit => unit.value === '斤' || unit.value === '公斤')
      } else {
        // 非散装模式下过滤掉斤和公斤
        return this.fullUnitList.filter(unit => unit.value !== '斤' && unit.value !== '公斤')
      }
  ...
  methods: {
    ...
    selectUnit(index, optIndex) {
      const selectedUnit = this.filteredUnitList[optIndex].value
      if (this.formData.isShareStock === 1) {
        this.formData.gradeList.forEach(grade => {
    ...
    changeSaleType(type) {
      ...
      if (type === 4) {
        // 散装模式：设置为斤
        this.formData.unit = '斤'
      } else {
        // 非散装模式：设置为件（如果当前单位是斤或公斤需要重置）
        if (this.formData.unit === '斤' || this.formData.unit === '公斤') {
          this.formData.unit = '件'
        }
      }
    ...
    // 选择单位回调
    editCardSpecSelct(e) {
      this.formData.unit = this.filteredUnitList[e].value
    },
    editCardSpecSelct1(e) {
      this.formData.specList[0].specName = this.filteredUnitList[e].label
    },
    editCardSpecSelct2(e) {
      this.formData.specList[1].specName = this.filteredUnitList[e].label
    },
    editCardSpecSelct3(e) {
      this.formData.specList[2].specName = this.filteredUnitList[e].label
    },
