<template>
	<div class="container2" style="position: relative;">
		<div class="vertical-tab">
			<VerticalTab ref="verticalTabRef" @update:activeTab="handleActiveTabUpdate" :categoryList="activeCategoryList"
				...
				<view class="search-container" style="width: calc(100% - 45px); max-width: 100%;">
					<view class="search-input-wrapper">
						...
						<view 
							...
							@click="clearSearch"
						>
							<uni-icons type="clear" size="16" color="#ccc" v-if="searchKeyword" ></uni-icons>
						</view>
					</view>
				</view>
			</view>
						...
						<div style="display: flex;margin-left: 15rpx;">
							<div v-for="(item, index) in row" :key="item.key">
								<div :id="'grid-item-'+item.id"
									:class="['grid-item', isItemActive(item) ? 'grid-item-active' : null]"
									...
									<div
										style="text-align: left;font-weight: bold;font-size: 24px;line-height: 1.2;margin-bottom: 2rpx;height: 30px;">
										{{ item.name }}
									</div>
									<div>
										<div class="commodity-tags">
											...
											<span v-if="item.isMultiLevel==0">
												...
												<span v-if="item.saleWay==2" class="tag tag-fixed">定{{item.initWeight?item.initWeight:''}}</span>
										...
										<div class="inventory-info"  v-if="item.isMultiLevel==0">
											...
											<span v-for="(item2, idx) in item.outPutPurchaseInventories" :key="idx" class="inventory-item">{{item2.mount}}{{item2.specName}}</span>
										</div>
									</div>
								</div>
							</div>
	...
	methods: {
		...
		updateGoodsList(list) {
			...
			for (let i = 0; i < sortedList.length; i += 3) {
				this.rows.push(sortedList.slice(i, i + 3));
			}
		...
		handleKeyWordClick(key) {
			...
			if (key === "全") {
				this.rows = [];
				const sortedList = [...this.commidityList].sort((a, b) => b.inventory - a.inventory);
				for (let i = 0; i < sortedList.length; i += 3) {
					this.rows.push(sortedList.slice(i, i + 3));
				}
			} else {
		...
		// 转换商品列表为行列格式（每行3个商品）
		convertToRows(commodityList) {
			const rows = [];
			for (let i = 0; i < commodityList.length; i += 3) {
				rows.push(commodityList.slice(i, i + 3));
			}
			return rows;
		},
		...
		finishLoadingCommodity(cacheKey) {
			this.rows = this.convertToRows(this.commidityList);
		...
		buildConsignmentCommodity(data, index) {
			return {
				...
				// 递归处理 childrenList
				childrenList: data.childrenList ? data.childrenList.map((child, childIndex) => this.buildConsignmentCommodity(child, childIndex)) : []
			};
		},
		...
		buildSelfOperatedCommodity(data, index) {
			return {
				...
				parentId: data.parentId,
				// 递归处理 childrenList
				childrenList: data.childrenList ? data.childrenList.map((child, childIndex) => this.buildSelfOperatedCommodity(child, childIndex)) : []
			};
		},
			...
			try {
				...
				if (this.activeVesionType === 4) {
					...
					if (this.internalCategoryList.length > 0) {
						// 检查当前是否有选中的
						const currentTabExists = this.currentTabId && 
							this.internalCategoryList.some(cat => cat.id === this.currentTabId);
		...
		async getConsignmentBatchList() {
			try {
				...
				if (this.activeVesionType !== 4 && batchList.length > 0) {
					// 检查当前批次是否还存在
					const currentTabExists = this.currentTabId && 
						batchList.some(cat => cat.id === this.currentTabId);
		...
		async getSelfOperatedCategoryList() {
			try {
				...
				if (this.activeVesionType !== 4 && categoryList.length > 0) {
					// 检查当前分类是否还存在
					const currentTabExists = this.currentTabId && 
						categoryList.some(cat => cat.id === this.currentTabId);
